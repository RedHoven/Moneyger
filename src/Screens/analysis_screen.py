...
class AnalysisScreen(Screen):
    ...
    def draw(self):
        ...
        headline = 'Statistics'
        self.analysis_screen.addstr(1, (self.main_width-len(headline))//2, headline)
        self.show_stats()
    ...
    def show_stats(self):
        ...
        this_month_headline = f'This Month Total:\t{data_this["total_spent"]}'
        ...
        self.analysis_screen.addstr(r, (c+self.main_width-center_block_width)//2, this_month_headline)
        ...
        self.analysis_screen.addstr(r+3,(c+self.main_width-center_block_width)//2,f'Total :\t\t')
        self.analysis_screen.addstr(f' {data_prev["total_spent"]}',color)
        ...
        self.analysis_screen.addstr(r+6,c+(blocksize-len('Most Frequent (times)'))//2,f'Most Frequent (times)')
        self.analysis_screen.addstr(r+6,c+blocksize+(blocksize-len('Most Expensive (EUR)'))//2,f'Most Expensive (EUR)')
        
        top_exp = list(zip(data_prev["top_5_spendings"].keys(), data_prev["top_5_spendings"].values()))
        top_freq = list(zip(data_prev["top_5_frequent"].keys(), data_prev["top_5_frequent"].values()))
        ...
        self.analysis_screen.addstr(r+15,c+(self.main_width-30)//2, f'Average per month:  {round(summary_data["avg_spendings_per_month"],2)}')
